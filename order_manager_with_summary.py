...
class OrderManager:
    def __init__(self, root):
        ...
        self.style.configure("Summary.TLabel", font=('Arial', 10))
        self.style.configure("SummaryHeader.TLabel", font=('Arial', 10, 'bold'))
    ...
    def create_widgets(self):
        ...
        # 金额汇总区域
        summary_frame = ttk.LabelFrame(main_frame, text="金额汇总", padding=10)
        summary_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=10)
        
        # 汇总标签
        ttk.Label(summary_frame, text="订单总金额:", style="SummaryHeader.TLabel").pack(anchor=tk.W, pady=(0,5))
        self.total_amount_label = ttk.Label(summary_frame, text="¥ 0.00", style="Summary.TLabel")
        self.total_amount_label.pack(anchor=tk.W, pady=(0,15))
        
        ttk.Label(summary_frame, text="按状态统计:", style="SummaryHeader.TLabel").pack(anchor=tk.W, pady=(0,5))
        
        self.pending_label = ttk.Label(summary_frame, text="待处理: ¥ 0.00", style="Summary.TLabel")
        self.pending_label.pack(anchor=tk.W, pady=2)
        
        self.completed_label = ttk.Label(summary_frame, text="已完成: ¥ 0.00", style="Summary.TLabel")
        self.completed_label.pack(anchor=tk.W, pady=2)
        
        self.cancelled_label = ttk.Label(summary_frame, text="已取消: ¥ 0.00", style="Summary.TLabel")
        self.cancelled_label.pack(anchor=tk.W, pady=2)
    ...
    def search_orders(self):
        ...
        # 更新汇总信息（基于搜索结果）
        self.update_summary(orders)
        
        self.status_var.set(f"搜索结果: {len(orders)} 条订单")
    
    def update_summary(self, orders=None):
        """更新金额汇总信息"""
        if orders is None:
            # 如果没有提供订单列表，则查询所有订单
            self.cursor.execute("SELECT total_amount, status FROM orders")
            orders = self.cursor.fetchall()
        else:
            # 从提供的订单列表中提取所需数据 (id, order_number, customer, date, amount, status, desc)
            orders = [(order[4], order[5]) for order in orders]
        ...
        self.pending_label.config(text=f"待处理: ¥ {pending:.2f}")
        self.completed_label.config(text=f"已完成: ¥ {completed:.2f}")
        self.cancelled_label.config(text=f"已取消: ¥ {cancelled:.2f}")
